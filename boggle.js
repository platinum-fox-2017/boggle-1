class Boogle {
  ...
  randomLibrary() {
    ...
    // let alfabetUnique = []
    // for(let i=0; i<joinAlfabet.length; i++) {
    //   if(!alfabetUnique.includes(joinAlfabet[i])) {
    //     alfabetUnique.push(joinAlfabet[i])
    //   }
    // }

    // let randomAlfabet = Math.floor((Math.random()*(alfabetUnique.length-1))+1)
    let randomAlfabet = Math.floor((Math.random()*(joinAlfabet.length-1))+1)

    // return alfabetUnique[randomAlfabet]
